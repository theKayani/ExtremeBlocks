...
{
	public static final ArrayList<IRecipe> PIES = new ArrayList<IRecipe>();
	...
	public static boolean canFitOnVanillaTable(ItemStack item, Object... obs)
	{
		...
		if(obs.length >= 4 && obs[3] instanceof String)
		{
			val = false;
		}
		...
		if(val == false)
		{
			...
			System.err.println("[" + (h < 10 ? "0" + h : h) + ":" + (m < 10 ? "0" + m : m) + ":" + (s < 10 ? "0" + s : s) + "] [main/ERROR] [ExtremeBlocks]: REMOVED '" + RecipeManager.add(false, item, obs).getRecipeOutput().getDisplayName() + "' RECIPE!");
		}
